...
export async function algorithmList(data) {
    try {
        console.log(data)
        const { pageNumber, pageSize, name, type, all } = data;
        const response = await service.get('/vision_algorithms');
...
export async function editItem(data) {
    try {
        const response = await service.put(`/vision_algorithms/${data.id}`, { sensitivity: data.sensitivity })
        console.log(response.data)
        return {
            code: 200,
        }
